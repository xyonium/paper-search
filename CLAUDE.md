...
### Docker Compose
`mcpo` (wrapping `paper-search-mcp`) and `open-webui` containers share the `paper-downloads` volume at `/downloads`:
...
Maps server name `"papers"` to `http://mcpo:8000/papers`:
...
| **直连**（绕后端） | hal, zhihuiya, patsnap, dblp, zenodo, pubmed, pmc, arxiv, semantic, openalex, crossref, europepmc, core, biorxiv, medrxiv, iacr, ieee, openaire, firecrawl | hal 走 `_hal_search`（绕后端 hal.py 的 isoformat bug）；hal 用 core，zhihuiya 用 distilled；dblp 走 `_dblp_search`（v2.5.3+，绕后端 dblp.py 的并发 ConnectionError + 无退避重试 bug）用 original（CS 书目，非 CS 查询 0 结果属正常）；zenodo 走 `_zenodo_search`（v2.5.3+，绕后端 zenodo.py 的 published_date str→isoformat crash bug）用 original；pubmed/pmc 走 `_pubmed_search`/`_pmc_search`（v2.6+，绕后端 pubmed.py 无 timeout 无限挂起 bug）用 original |
...
**iacr 移出字面组与默认源**（v2.6）：iacr 无需精简——密码学主题词实测 2-8 词全部 ≥3 命中（ePrint 全文索引，相关性宽松）；之前 04:16 日志的 0 命中是"非密码学查询必然 0"（`continuous monitoring biofouling...`），不是词数问题。iacr 是密码学 ePrint **细分库**（非覆盖完整大学科），默认源应覆盖完整学科（计算机/生物/医学等），故移出默认；需要密码学文献时 `sources=iacr` 显式调用（v2.9 起直连 `_iacr_search`：ePrint 无 JSON API，正则解析搜索页 HTML，Xapian 词干 AND，按 ID 倒序=最新优先，用 original 查询）。
...
- `all` 模式下后端源用 `_BACKEND_ALL_SOURCES`（v2.9 起仅剩 doaj/google_scholar/ssrn/unpaywall/citeseerx/base/acm——主力源全部直连后，后端定位降级为安全网）；拆分时语义组用 `_SEMANTIC_ALL_SOURCES`（再排除字面组 doaj）。
- **v2.9 直连批次二**（`_semantic_search`/`_openalex_search`/`_crossref_search`/`_europepmc_search`/`_core_search`/`_rxiv_search`/`_iacr_search`，共用 `_http_get` 三次退避，429/5xx 重试、其余 4xx 立即失败）：semantic 遵守 Retry-After、403 自动降匿名（valve `semantic_api_key`）；core 401/403 降匿名（valve `core_api_key`）；openalex 摘要是倒排索引需按位置重建；crossref title 是 list、abstract 带 JATS 标签；europepmc id 按 source 分 MED→`pmid:`/PMC→`pmc:`；biorxiv/medrxiv 是近30天学科浏览（非关键词检索），显式点名才启用、all 模式需带 category；iacr 正则解析 HTML（不引入 bs4）。**read_paper 调后端 read 工具前会剥离直连源自家前缀**（`arxiv:`/`semantic:`/`iacr:` 等），修复前缀 id 击穿后端 fast path 的隐患。
- **v2.9.1 AND 源逐级放宽**（`_relax_and`）：hal(Solr)/europepmc/pubmed/pmc(eutils)/openaire(keywords=)/ieee(querytext) 把空格分隔词当 AND，11 词自然语言查询实测全 0 命中（hal  distilled-5→0 vs 2 词→61；europepmc 11 词→0 vs 3 词→55）。统一处理：>6 词先 `_distill_core_terms` 蒸馏，再 AND 前 min(len,5) 词，0 命中砍尾词逐级重试到 1 词，首个非空即返回（arxiv 同款先例）。用户 PEGDGE 11 词查询实测：hal 0→1、pubmed 0→3、pmc 0→5、europepmc 0→5。**dblp 反爬识别**：2026-09 起 dblp 上 Anubis 质询页（200+text/html "Making sure you're not a bot"），直连版检测 200 非 JSON → 立即 RuntimeError「疑似反爬拦截」（持续拦截不重试），不再报误导性的 "Expecting value" JSON 解析错。**逐源实况冒烟** `scripts/live_sources.py`：每源一个稳定查询，串行跑 PASS/FAIL/EMPTY+命中数+耗时，key 源从环境变量注入（SEMANTIC_API_KEY/IEEE_APIKEY/CORE_API_KEY/ZENODO_ACCESS_TOKEN/ZHIHUIYA_APIKEY/NCBI_API_KEY/ANTIBOT_PROXY_URL），用于区分网络波动/限流/反爬 vs 代码 bug。2026-09 实测基线：宿主机 arxiv 429/semantic 429（共享 IP 限流，配 key 后 PASS）/dblp 反爬/zenodo 504/openaire 超时（api.openaire.eu 当时整体不可达，容器内同样超时）均属环境；hal/pubmed/pmc/openalex/crossref/europepmc/core/biorxiv/medrxiv/iacr 全 PASS。
- **v2.9.3 反爬真实解法（替代 v2.9.2 住宅代理设计，已回滚）**：Anubis 是 JS PoW 质询，**headless 浏览器天然可解，跟 IP 信誉无关**——所以兜底走自托管 firecrawl（`_dblp_via_firecrawl`：检出 `_AntiBotBlocked` 后调 `firecrawl_scrape` rawHtml+waitFor 8s，解析 `<pre>` 或直接 JSON body 两种形态都兼容——hosted 侧返回 `<pre>` 包装、mcpo 侧 Content-Type=application/json 原样透传，实测均穿透，~5s）。**google_scholar 可改走 Apify actor**（valve `apify_rotator_base_url`，如 http://api-key-rotator:8788；POST `/v2/acts/johnvc~google-scholar-api/run-sync-get-dataset-items` `{q, maxResults, mode:"search"}`，201 同步返回）：实测 5.9s 返回完整结构化结果（paper_title/publication_info.authors/snippet/inline_links.cited_by_total/result_id），免费层可用但结果数受限（PAY_PER_EVENT）。配了就把 scholar 从后端 sources 剔除（含 all_mode 的 _BACKEND_ALL_SOURCES/_SEMANTIC_ALL_SOURCES 动态剔除）；未配保持后端路径。**Apify store 实测（2026-09）**：dblp actor 全废（parseforge 版自己撞 Anubis 报 JSON 解析错、logiover 版跑挂）——Anubis 站点只有真浏览器好使；scholar actor 多家可选（johnvc/scrape.badger 等）。api-key-rotator 的 apify 通路（/v2/acts 转发+token 轮转）实测工作正常；其住宅代理是付费 plan 功能且 proxy password≠API token（407），故弃用。无需自研 actor：firecrawl=Anubis 类站点的自托管零成本"actor"，scholar 有现成 actor。
- **v2.9.4 google_scholar 三级链**（用户拍板：firecrawl 首选 → apify actor 兜底 → 都没配才走后端）：`_google_scholar_firecrawl_search` 用 `_mcp_call_service_url(firecrawl_base_url, "firecrawl_scrape", {formats:["markdown"]})` 抓 `scholar.google.com/scholar?q=...&hl=en&num=N` 搜索页，`_parse_scholar_markdown` 按 `### [title](url)` 块解析。**scholar markdown 解析要点（2026-09 实测两种形态）**：作者行分隔符带 NBSP（`作者…\xa0- 期刊, 2024 - 域名`，解析前必须 `replace("\xa0"," ")` 否则 split(" - ") 切错位置）；作者两种形态（citations 用户链接 `[Name](...citations?user=)` 或纯文本逗号分隔）；行动线两种形态（`[Save](javascript:..) [Cite](..)` 或纯文本 `SaveCite`）；snippet 可多行；标题内嵌 `**高亮**`；行首可带 `\[PDF\]/\[HTML\]` 字面标签。检测到 "unusual traffic" → `_AntiBotBlocked`。**实测通路矩阵**：tavily /extract 通但内容薄（只有标题+链接）、exa /contents 被 400 拒（CRAWL_HTTP_400）、jina 被 CAPTCHA（IP 黑名单）、自托管 firecrawl 看 IP 运气、**mcpo→rotator→官方 firecrawl 云稳定穿透（2.3s 完整 markdown）**、apify actor 稳定（5.9s）。firecrawl 出空结果且配了 actor 时也落 actor（空页多半是被软拦）。
- **v2.9.5 scholar 链插入 tavily 第二级**（`_google_scholar_tavily_search`）：`/extract` 必须 `extract_depth=advanced`（默认 basic 只出标题+链接无作者/引用）。链改为 firecrawl → tavily(advanced) → Apify actor（PAY_PER_EVENT 付费放最后）→ 都没配走后端；`_gscholar` 重构为 attempts 列表逐级下落（前级被拦/失败/空结果自动落下一级，全失败抛最后异常）。**tavily advanced 怪癖（解析器已兼容）**：吃掉 `**` 两侧空格（`models**with`，`_clean_md_text` 把 `*` 换成空格再折叠）；作者行分隔符无空格（`Hong…- Nature`，改用 `\s*[-–]\s+|\s+[-–]\s*` 正则切，连字符结尾作者名 "Machta-" 不误切）；PDF 链接不转义（`[[PDF] x](url)`）。实测（2026-09）：firecrawl 1.3s / tavily 4.6s 均 5 命中字段完整。
