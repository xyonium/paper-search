...
zhihuiya/ieee 在默认列表中，但**必须同时满足两个条件才启用**：(a) 源在 `default_sources`（或 `all` 模式、或调用时显式 `sources=`）；(b) 配了对应 apikey。即配 key 是必要非充分条件——未配 key 时该源静默跳过（`want_zh`/`want_ieee` 为 False）。google_scholar 同批加回默认（无 proxy 可能 403，失败自动降级不影响整体）。firecrawl 不在默认列表（需配 `firecrawl_base_url` + 显式 `sources=` 或 `all` 模式）。
...
- **web 搜索双轨**（v2.5.4）：
  - **firecrawl = 独立源**（不是 fallback）：配 `firecrawl_base_url`（如 `http://mcp:8000/firecrawl`）+ sources 含 `firecrawl`（或 `all` 模式）才启用。走 mcpo `firecrawl_research_search_papers`（firecrawl 里唯一的学术文献专用端点，`firecrawl_search` 是通用 web 搜索不适合）。**无域名限定**（research 接口不支持 include_domains），查询用 **core 变体**（去噪声词，firecrawl 内部有查询处理故不截断）。结果 `source="firecrawl"`，`paper_id` 保留原 id（如 `arxiv:xxx`，实测 5 条全中高相关）。定位：作为主链源参与检索，补 API 源覆盖不到的网页文献。
  - **tavily = 其他源的 fallback**（不是独立源）：配 `tavily_base_url`（如 `http://api-key-rotator:8788/tavily`，经 api-key-rotator 代理 key 池轮转）才启用。任一**请求的源**出现连接/超时类错误（匹配 超时/timeout/ssl/eof/connection/502/503/504/429；0 命中或 400 参数错不触发）时触发，`POST {tavily_base_url}/search` 用 **original** 查询 + `include_domains` 限定 12 个学术站（arxiv/ieee/aclanthology/semanticscholar/openreview/acm/springer/sciencedirect/pubmed/nature/biorxiv/medrxiv）。结果 `source="tavily"`，`paper_id` 从 URL 提取（arxiv/ieee/pubmed/aclanthology/doi/biorxiv 模式），doi 另从 `raw_content` 正则回填。定位：只在主链网络类失败时精准补位。**tavily vs firecrawl 实测对比**（2026-08，同查询）：tavily 结构化 JSON + 域名限定，相关性更高，故做 fallback；firecrawl 无域名限定但文献专用端点覆盖广，故做独立源。注意 tavily `include_domains` 为软提示（服务端可能返回域名外结果），api-key-rotator 原样透传请求体不干预。
