...
- **v2.9.2 反爬住宅代理兜底**：`_AntiBotBlocked(RuntimeError)` 标记异常——源明确检出反爬拦截页（当前 dblp：200+非JSON Anubis 质询页）时，若配 valve `antibot_proxy_url`（仅管理员级，如 `http://groups-RESIDENTIAL:<proxy密码>@proxy.apify.com:8000`）则换代理+浏览器 UA 重试**一次**（住宅代理月度额度有限，仅此场景触发；仍被拦则报「代理重试仍被反爬拦截」）。**Apify 实测坑（2026-09）**：proxy.apify.com:8000 用 API token 当密码 → 407，必须用控制台 Proxy 页的独立 proxy password；api-key-rotator 的 apify 转发只是 API 转发（/v2/acts→api.apify.com query-param token），不是 CONNECT 代理，帮不上反爬场景。**semantic 429 根因实测**：无效 key→403（触发降匿名正确）；真 key 连发 3 个（含间隔 2s）→ 第 3 个 429 且**无 Retry-After 头**——S2 配额是长窗口（~100req/5min），OWUI tool 与 paper-search-mcp 共用同一把 key（config.json PAPER_SEARCH_MCP_SEMANTIC_SCHOLAR_API_KEY）互相挤占 → 解法是给 tool 申请独立 S2 key。**google_scholar 代理**：后端 google_scholar.py 原生支持 `PAPER_SEARCH_MCP_GOOGLE_SCHOLAR_PROXY_URL`（当前指向局域网 192.168.254.253:1090），要换住宅代理只需改 config.json 重启后端，无需改代码。
